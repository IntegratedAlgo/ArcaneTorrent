...
pub async fn get_search_results(configuration: &configuration::Configuration, query: Option<&str>, q: Option<&str>, r#type: Option<&str>, year: Option<f32>, company: Option<&str>, country: Option<&str>, director: Option<&str>, language: Option<&str>, primary_type: Option<&str>, network: Option<&str>, remote_id: Option<&str>, offset: Option<f32>, limit: Option<f32>) -> Result<crate::models::GetSearchResults200Response, Error<GetSearchResultsError>> {
    ...
    if let Some(ref local_var_str) = director {
        local_var_req_builder = local_var_req_builder.query(&[("director", &local_var_str.to_string())]);
    }
    ...
    if let Some(ref local_var_str) = primary_type {
        local_var_req_builder = local_var_req_builder.query(&[("primaryType", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetSearchResultsError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn get_search_results_by_remote_id(configuration: &configuration::Configuration, remote_id: &str) -> Result<crate::models::GetSearchResultsByRemoteId200Response, Error<GetSearchResultsByRemoteIdError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/search/remoteid/{remoteId}", local_var_configuration.base_path, remoteId=crate::apis::urlencode(remote_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetSearchResultsByRemoteIdError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
